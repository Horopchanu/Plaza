...
(function ($) {
  ...
  var Mapplic = function Mapplic(element) {
    ...
    self.init = function (options) {
      ...
      self.el.addClass('mapplic-element mapplic-loading'); // scope

      if (self.o.scope) self.scope = $(self.o.scope);else self.scope = self.el; // process map data
    ...
    function Tooltip() {
      ...
      this.init = function (location, check) {
        ...
        this.triangle = $('<div></div>').addClass('mapplic-tooltip-triangle').prependTo(this.wrap); // no focus on deeplink
        ...
        if (location) this.show(location); // close tooltip
      ...
      this.repos = function (e) {
        ...
        if (rleft < s.wrap.outerWidth() / 2) {
          if (rleft > 12) tx = -(100 + rleft / s.wrap.outerWidth() * 100) + 100 + "%";else tx = "-10%";
        } else if (rleft > self.container.el.outerWidth() - s.wrap.outerWidth() / 2) {
          if (rleft < self.container.el.outerWidth() - 12) tx = (self.container.el.outerWidth() - rleft) / s.wrap.outerWidth() * 100 - 100 + "%";else tx = "-90%";
        } else tx = "-50%";
      ...
      this.show = function (location) {
        if (location) {
          ...
          this.content[0].scrollTop = 0; // shift

          this.pin = $('.mapplic-pin[data-location="' + location.id + '"]');

          if (this.pin.length == 0) {
            this.shift = 20;
          } else this.shift = Math.abs(parseFloat(this.pin.css('margin-top'))) + 20; // making it visible
          ...
          this.position();
          if (self.o.zoom) this.zoom(location); // loading & positioning
    ...
    function HoverTooltip() {
      ...
      this.init = function () {
        var s = this; // construct

        this.el = $('<div></div>').addClass('mapplic-tooltip mapplic-hovertip');
        this.wrap = $('<div></div>').addClass('mapplic-tooltip-wrap').appendTo(this.el);
        this.title = $('<h4></h4>').addClass('mapplic-tooltip-title').appendTo(this.wrap);
        if (self.o.hovertipdesc) this.desc = $('<div></div>').addClass('mapplic-tooltip-description').appendTo(this.wrap);
        this.triangle = $('<div></div>').addClass('mapplic-tooltip-triangle').appendTo(this.wrap); // events 
        // markers + old svg
    ...
    function Lightbox() {
      ...
      this.init = function () {
        ...
        if (self.o.linknewtab) this.link.attr('target', '_blank'); // append
    ...
    function DeeplinkingHash() {
      ...
      this.init = function () {
        var s = this;
        this.check(0);
        $(window).on('hashchange', function () {//s.check(600);
        });
      };
    ...
    function Directory() {
      ...
      this.init = function () {
        ...
        self.el.on(event, function (e, self) {
          ...
          $('.mapplic-dir-filter').each(function () {
            ...
            $(this).on('change', function () {
              if (this.value) s.filters[attribute] = this.value;else delete s.filters[attribute];
              s.search();
            });
          });
        }); // search
        ...
        $('.mapplic-dir-search').keyup(function (e) {
          var val = $(this).val();
          if (val) s.filters['keyword'] = val;else delete s.filters['keyword'];
          s.search();
          if (e.keyCode == 13) $('.mapplic-dir-results li > a').filter(':first').click();else if (e.keyCode == 27) s.clearFilters();
        }); // results
      ...
      this.getItem = function (location) {
        ...
        var link = $('<a></a>').attr('href', '#').click(function (e) {
          e.preventDefault();
          self.showLocation(location.id, 600); // scroll back to map on mobile
      ...
      this.thumbnail = function (name, field) {
        ...
        if (field) {
          if (field.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g)) elem = $('<img>').attr('src', field).attr('alt', name).addClass('mapplic-thumbnail');else elem = $('<div></div>').addClass('mapplic-thumbnail mapplic-thumbnail-placeholder').attr('aria-hidden', 'true').html(field);
        } else if (name) {
      ...
      this.normalizeString = function (s) {
        if (s) return s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "");else return '';
      };
      ...
      this.init = function () {
        ...
        if (self.o.search) {
          ...
          this.input = $('<input>').attr({
            ...
            var val = $(this).val();
            if (val) self.directory.filters['keyword'] = val;else delete self.directory.filters['keyword'];
            s.el.toggleClass('mapplic-sidebar-header-opened', val.length < 1);
            s.search();
            if (e.keyCode == 13) $('.mapplic-list-container li > a').filter(':visible:first').click();else if (e.keyCode == 27) $(this).blur();
          });
          ...
          this.toggle = $('<button></button>').append(getIcon('icon-filter')).addClass('mapplic-search-toggle').click(function (e) {
            e.preventDefault();
            s.el.toggleClass('mapplic-sidebar-header-opened');
          }).appendTo(this.headerwrap); // tags

          this.tags = $('<div></div>').addClass('mapplic-filter-tags').appendTo(this.headerwrap); // filters

          this.filter = $('<div></div>').addClass('mapplic-filter').appendTo(this.header); // dim

          this.dim = $('<div></div>').addClass('mapplic-sidebar-dim').click(function () {
            s.el.removeClass('mapplic-sidebar-header-opened');
          }).appendTo(this.el);
        } else this.el.addClass('mapplic-sidebar-nosearch'); // items


        var event = 'mapready';
        if (self.o.csv) event = 'csvready';
        self.el.on(event, function (e, self) {
          s.dir = self.directory.getDirectory().addClass('mapplic-list-container').appendTo(s.el); // alphabetic sort by title
      ...
      this.addTag = function (item) {
        ...
        if (!categories.includes(item.id)) {
          ...
          tag.click(function () {
            ...
            if (self.directory.filters['category'] == item.id) delete self.directory.filters['category'];else if (self.directory.filters['category']) {
        ...
        if (categories) {
          $.each(categories, function (index, category) {
            ...
            if (!(category.hide == 'true')) {
              ...
              if (!category.about) title.addClass('mapplic-margin');else $('<span></span>').addClass('mapplic-about').html(category.about).appendTo(link);
          ...
          if (add) {
            s.el.addClass('mapplic-sidebar-filterable'); //$('<h5></h5>').text('CATEGORIES').appendTo(s.filter);
    ...
    function ZoomButtons() {
      ...
      this.init = function () {
        this.el = $('<div></div>').addClass('mapplic-zoom-buttons').appendTo(self.container.el); // zoom-in button
      ...
      this.update = function (scale) {
        ...
        if (scale == self.fitscale) this.zoomout.attr('disabled', 'disabled');else if (scale == 1) this.zoomin.attr('disabled', 'disabled');
      };
    } // fullscreen
    ...
    function Fullscreen() {
      ...
      this.init = function () {
        ...
        this.eph = $('<div></div>').addClass('mapplic-esh').insertBefore(self.el); // fullscreen button

        this.el = $('<button></button>').append(getIcon('icon-fullscreen')).append(getIcon('icon-fullscreen-exit')).addClass('mapplic-button mapplic-fullscreen-button').click(function (e) {
          self.el.toggleClass('mapplic-fullscreen');
          if (self.el.hasClass('mapplic-fullscreen')) self.el.insertBefore(s.fph);else self.el.insertBefore(s.eph);
          $(document).resize();
        }).appendTo(self.container.el); // esc key
    ...
    function Styles() {
      ...
      this.process = function (styles) {
        ...
        if (self.o.textcolor) css += this.rule('.mapplic-element, .mapplic-element a, .mapplic-level-switcher button, .mapplic-about, .mapplic-list-category > a .mapplic-list-count, .mapplic-search-input::placeholder, .mapplic-lightbox-description', 'color', self.o.textcolor); // accentcolor
    ...
    function Container() {
      ...
      this.revealZoom = function (zoom) {
        $('.mapplic-pin[data-reveal]', self.map).each(function () {
          ...
          if (zoom * self.o.maxscale >= reveal) $(this).addClass('mapplic-revealed');else $(this).removeClass('mapplic-revealed');
        });
      };
      ...
      this.addControls = function () {
        ...
        if (self.o.mousewheel) $('.mapplic-map-image', self.el).on('mousewheel DOMMouseScroll', self.mouseWheel); // touch
    ...
    var processData = function processData(data) {
      ...
      self.g = {};
      self.l = {};
      var shownLevel = null; // extend options
      ...
      self.o.maxscale = parseFloat(self.o.maxscale); // more text

      if (self.o.moretext) self.loc.more = self.o.moretext; // height of container

      if (self.el.data('height')) self.o.height = self.el.data('height');
      self.contentWidth = parseFloat(data.mapwidth);
      self.contentHeight = parseFloat(data.mapheight); // limiting to scale 1

      self.contentWidth = self.contentWidth * self.o.maxscale;
      self.contentHeight = self.contentHeight * self.o.maxscale; // create container

      self.container = new Container().init(); // styles

      self.styles = new Styles().init(self.o.styles); // create minimap

      if (self.o.minimap) self.minimap = new Minimap().init(); // create legend

      self.legend = new Legend().init();
      self.legend.build(data.groups || data.categories); // directory

      self.directory = new Directory().init(); // create sidebar

      if (self.o.sidebar) {
        self.sidebar = new Sidebar();
        self.sidebar.init();
      } else self.container.el.css('width', '100%'); // groups


      self.groups = new Groups();
      self.groups.addGroups(data.groups || data.categories); // trigger event

      self.el.trigger('mapstart', self); // iterate through levels
      ...
      if (data.levels) {
        $.each(data.levels, function (index, level) {
          var source = level.map,
              extension = source.substr(source.lastIndexOf('.') + 1).toLowerCase(); // new map layer
          ...
          switch (extension) {
            ...
            case 'svg':
              ...
              $('<div></div>').load(source, function () {
                ...
                $('script', this).remove();
                mapimage.html($(this).html()); // illustrator duplicate id fix
                ...
                if (level.locations) self.addLocations(level.locations, level.id); // click event
          ...
          if (self.minimap) self.minimap.addLayer(level); // shown level
      ...
      if (self.o.portrait === 'true') self.o.portrait = true; // resize

      $(window).resize(function () {
        if (self.o.portrait == true || $.isNumeric(self.o.portrait) && self.el.width() < parseFloat(self.o.portrait)) {
          self.el.addClass('mapplic-portrait');
          if (self.el.hasClass('mapplic-fullscreen')) self.container.el.height($(window).height());else self.container.el.height(self.container.calcHeight(self.o.height));
        } else {
      ...
      if (self.o.deeplinking) {
        if (history.pushState && typeof URL == 'function') self.deeplinking = new Deeplinking();else self.deeplinking = new DeeplinkingHash();
        self.deeplinking.init();
      } // trigger event


      if (toload == 0) mapReady(); // controls

      if (self.o.zoom) self.container.addControls();
      self.firstcoord = self.lastcoord = {}; // link to locations
    ...
    var mapReady = function mapReady() {
      ...
      self.addLocations(self.data.locations); // trigger event

      self.el.trigger('mapready', self); // apply toggle

      self.legend.applyToggles(); // CSV support
      ...
      self.container.resetZoom();
      if (self.deeplinking) self.deeplinking.check(0); // landmark mode
    ...
    var normalizeScale = function normalizeScale(scale) {
      ...
      if (scale <= self.fitscale) scale = self.fitscale;else if (scale > 1) scale = 1; // zoom timeout
    ...
    var zoomTo = function zoomTo(x, y, scale, d) {
      d = typeof d !== 'undefined' ? d / 1000 : 0; // move class
      ...
      if (self.minimap) self.minimap.update(x, y); // trigger event

      self.el.trigger('positionchanged', location);
    };

    var replaceVars = function replaceVars(template, location) {
    ...
    self.switchLevel = function (target) {
      // no such layer
      if (!target) return;
      var layer = $('.mapplic-layer[data-floor="' + target + '"]', self.map); // target layer is already active

      if (layer.hasClass('mapplic-visible')) return; // show target layer
      ...
      layer.removeClass('mapplic-hidden').addClass('mapplic-visible'); // slide animation
      ...
      self.level = target; // trigger event
    ...
    self.addLocation = function (location, levelid) {
      ...
      if (self.l[location.id]) return true; // building the location object

      self.l[location.id] = location; // groups
      ...
      var fill = location && location.fill || location.category && self.g[location.category] && self.g[location.category].color || location.category && location.category[0] && self.g[location.category[0]] && self.g[location.category[0]].color || self.o.fillcolor || false; // cascade style to attribute styleD!

      location.styled = location && location.style || location.category && self.g[location.category] && self.g[location.category].style || location.category && location.category[0] && self.g[location.category[0]] && self.g[location.category[0]].style || self.o.defaultstyle || false;
      if (fill) location.color = fill;else if (location.styled) location.color = location.styled.base; // interactive element
      ...
      if (!location.level) {
        if (levelid) location.level = levelid;else if (location.el && location.el.closest('.mapplic-layer').data('floor')) location.level = location.el.closest('.mapplic-layer').data('floor');else location.level = self.data.levels[0].id;
      } // description vars


      if (location.description) location.description = replaceVars(location.description, location); // geolocation
      ...
      location.marker = self.addMarker(location); // reveal mode
    ...
    self.addMarker = function (location) {
      ...
      if (location.label) {
        if (location.label.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g)) marker.css('background-image', 'url(' + location.label + ')');else $('<span><span>' + location.label + '</span></span>').appendTo(marker);
      }
      ...
      if (location.color && location.pin.indexOf('pin-text') > -1) marker.css('color', location.color);else if (location.color) marker.css({
    ...
    self.removeLocations = function () {
      for (var _i = 0, _Object$keys = Object.keys(self.l); _i < _Object$keys.length; _i++) {
    ...
    self.showLocation = function (id, duration, check) {
      ...
      if (action == 'disabled') return false; // trigger event
      ...
      self.location = self.l[id]; // active state

      $('.mapplic-active', self.scope).removeClass('mapplic-active');
      if (location.el) location.el.addClass('mapplic-active');
      if (location.list) location.list.addClass('mapplic-active'); // deeplinking

      if (self.deeplinking && !check) self.deeplinking.update(id); // trigger event
  ...
  $.fn.mapplic = function (options) {
    return this.each(function () {
      var element = $(this); // plugin already initiated on element

      if (element.data('mapplic')) return;
      var instance = new Mapplic(element).init(options); // store plugin object in element's data

      element.data('mapplic', instance);
    });
  };
})(jQuery); // call plugin on map instances


jQuery(document).ready(function ($) {
  $('[id^=mapplic-id]').mapplic(); // dynamic element
