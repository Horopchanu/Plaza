...
var CSSMatrix = function CSSMatrix(str) {
...
function setMatrixValue(str) {
  ...
  var values = str.match(/\d*[.]?\d+(?=,|\))/g); // set the 3D values this.m11 - this.m44
...
CSSMatrix.prototype = {
   ...
   */
  rotate: function rotate(rotX, rotY, rotZ) {
    ...
    rotY = rotY || rotX;
    rotZ = rotZ || rotZ; // TODO optimize this with multiplication by a single matrix
   ...
   */
  __rotateX__: function __rotateX__(angle) {
   ...
   */
  __rotateY__: function __rotateY__(angle) {
   ...
   */
  __rotateZ__: function __rotateZ__(angle) {
    var c = Math.cos(-angle * this.PID180),
        s = Math.sin(-angle * this.PID180),
        m = this.__clone__();
   ...
   */
  rotateAxisAngle: function rotateAxisAngle(x, y, z, angle) {
    y = y || x;
    z = z || y; // Optimize the rotation transformation by using specific rotation functions when possible
   ...
   *
   */
  setMatrixValue: function setMatrixValue(str) {
    // when a string is provided, parse it for the matrix values
    var values = str.match(/\d*[.]?\d+(?=,|\))/g); // set the 3D values this.m11 - this.m44
   ...
   */
  __determinant__: function __determinant__() {
