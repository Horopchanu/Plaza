...
<div class="tenants-alphabet__block  -added">
 ...
 <div class="tenants-alphabet__col -result">
  ...
  const emptySearch = `  <div class="ticker ui-section">
    ...
    tenantsSearchInput.addEventListener("input", function () {
      ...
      if (this.value) {
        ...
        searchTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            updateResults(emptySearch);
            // подставить бегущие строки
            console.log(document.querySelector(".tenants-view__filter"));
            document
              .querySelectorAll(".tenants-view__filter")
              .forEach(function (element) {
                element.classList.add("-disabled");
              });
            document
              .querySelector(".tenants-result")
              .classList.add("-search-empty");
            setTimeout(() => {
              ...
              if (tickerLines.length && !isIE11) {
                window.requestAnimationFrame(checkOffset);
                const tickerItemLinks =
                  document.querySelectorAll(".ticker__item-link");
                document.addEventListener("mouseover", function (event) {
                  if (event.target.closest(".ticker__item-link")) {
                    event.target
                      .closest(".ticker__line")
                      .classList.add("js-hover");
                  }
                });
